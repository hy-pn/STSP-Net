...
class PredOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--pred_dir', required=False, default=''+DATA_NAME, help='directory to output masks')
        parser.add_argument('--chkpt_path', required=False, default=working_path+'/checkpoints/'+DATA_NAME+'.pth')
...
def predict(net, pred_set, pred_loader, pred_dir, flip=False, index_map=False, intermediate=False):
    ...
    if index_map:
        pred_A_dir = os.path.join(pred_dir, 'label1')
        pred_B_dir = os.path.join(pred_dir, 'label2')
    ...
    for vi, data in enumerate(pred_loader):
        ...
        io.imsave(pred_A_path, pred_A.astype(np.uint8)*255)
