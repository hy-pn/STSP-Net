...
def rand_flip_MCD_mask(img1, img2, label1, label2, labelmask):
    r = random.random()
    # showIMG(img.transpose((1, 2, 0)))
    if r < 0.25:
        return img1, img2, label1, label2, labelmask
    elif r < 0.5:
        return np.flip(img1, axis=0).copy(), np.flip(img2, axis=0).copy(), np.flip(label1, axis=0).copy(), np.flip(label2, axis=0).copy(), np.flip(labelmask, axis=0).copy() 
    elif r < 0.75:
        return np.flip(img1, axis=1).copy(), np.flip(img2, axis=1).copy(), np.flip(label1, axis=1).copy(), np.flip(label2, axis=1).copy(), np.flip(labelmask, axis=1).copy()
    else:
        return img1[::-1, ::-1, :].copy(), img2[::-1, ::-1, :].copy(), label1[::-1, ::-1].copy(), label2[::-1, ::-1].copy(), labelmask[::-1, ::-1].copy()
...
def rand_rot90_MCD_mask(img1, img2, label1, label2, labelmask):
    r = random.random()
    # showIMG(img.transpose((1, 2, 0)))
    if r < 0.5:
        return img1, img2, label1, label2, labelmask
    else:
        return np.rot90(img1).copy(), np.rot90(img2).copy(), np.rot90(label1).copy(), np.rot90(label2).copy(), np.rot90(labelmask).copy()
...
def rand_rot90_flip_MCD_mask(img1, img2, label1, label2, labelmask):
    img1, img2, label1, label2, labelmask = rand_rot90_MCD_mask(img1, img2, label1, label2, labelmask)
    return rand_flip_MCD_mask(img1, img2, label1, label2, labelmask)
...
def data_padding(imgs, labels, scale=32):
    for idx, img in enumerate(imgs):
        ...
        if need_padding:
            ...
            img = util.pad(img, ((h_padding1, h_padding2), (w_padding1, w_padding2), (0,0)), 'symmetric')
            label = util.pad(label, ((h_padding1, h_padding2), (w_padding1, w_padding2), (0,0)), 'constant')
...
def data_padding_fixsize(imgs, labels, size):
    for idx, img in enumerate(imgs):
        ...
        img = util.pad(img, ((h_padding1, h_padding2), (w_padding1, w_padding2), (0,0)), 'symmetric')
        label = util.pad(label, ((h_padding1, h_padding2), (w_padding1, w_padding2)), 'constant')
        imgs[idx] = img
        labels[idx] = label
    return imgs, labels
