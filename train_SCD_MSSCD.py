...
from utils.utils import accuracy, SCDD_eval_all_MSSCD, AverageMeter
...
def main():        
    ...
    train_loader = DataLoader(train_set, batch_size=args['train_batch_size'], num_workers=4, shuffle=True,drop_last=True)
    val_set = RS.Data('val')
    ...
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            if args['gpu']:
                ...
                labels_bn = (labels_B>0).unsqueeze(1).cuda().float()
            ...
            loss_bn = weighted_BCE_logits(out_change, labels_bn, weight_pos=0.1, weight_neg=0.9)
            ...
            outputs_A = outputs_A.detach()
            outputs_B = outputs_B.detach()
...
def validate(val_loader, net, criterion, curr_epoch):
    ...
    torch.cuda.empty_cache()
    start = time.time()
    # criterion1 = CrossEntropyLoss2d(ignore_index=0).cuda()
    ...
    for vi, data in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            # loss_seg3 = criterion1(outputs_B1,labels_B)
        ...
        for (pred_A, pred_B, label_A, label_B) in zip(preds_A, preds_B, labels_A, labels_B):
            acc_A, valid_sum_A = accuracy(pred_A, label_A)
            acc_B, valid_sum_B = accuracy(pred_B, label_B)
            preds_all1.append(pred_A)
            preds_all2.append(pred_B)
            labels_all1.append(label_A)
            labels_all2.append(label_B)
            acc = (acc_A + acc_B)*0.5
            acc_meter.update(acc)

    if len(preds_all2)!=len(labels_all2):
        raise Exception("Picture group number is mismatch.")
    score, IoU_mean, Sek ,Fscd,F1,kappa= SCDD_eval_all_MSSCD(preds_all1,preds_all2, labels_all1,labels_all2, 2,8)

    curr_time = time.time() - start
    print('%.1fs Val loss: %.2f Score: %.2f IoU: %.2f Sek: %.2f Accuracy: %.2f Fscd: %.2f F1: %.2f kappa: %.2f'\
    %(curr_time, val_loss.average(), score*100, IoU_mean*100, Sek*100, acc_meter.average()*100, Fscd*100,F1*100,kappa*100))
